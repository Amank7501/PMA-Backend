...
import com.rasp.app.helper.LabelIssueMapHelper;
...
public class IssueUserDecorator extends BaseDecorator {
    public IssueUserDecorator() {
        super(new IssueUserMap());
    }
...
//    }
    ...
    public BaseResource[] getQuery(ServletContext ctx, String queryId, Map<String, Object> map, BaseService service) throws ApplicationException {
        if("GET_USER_BY_ISSUE_ID".equalsIgnoreCase(queryId)){
            String issue_id = (String) map.get(Issue.FIELD_ID);
            if (Util.isEmpty(issue_id)) {
                throw new ApplicationException(ExceptionSeverity.ERROR, "Issue id not found!!!");
            }
            Expression e1 = new  Expression(IssueUserMap.FIELD_ISSUE_ID, REL_OP.EQ, issue_id);
            BaseResource[] issueUser = IssueUserMapHelper.getInstance().getByExpression(e1);
            ArrayList<BaseResource> userList = new ArrayList<>();
            for (BaseResource iu : issueUser) {
                IssueUserMap issueusermap = (IssueUserMap) iu;
                Expression e2 = new Expression(User.FIELD_ID, REL_OP.EQ, issueusermap.getUser_id());
                BaseResource[] userHelper = UsersHelper.getInstance().getByExpression(e2);

                if (userHelper != null && userHelper.length > 0) {
                    for (BaseResource u : userHelper) {
                        userList.add(u);
                    }
                }
            }
       ...
       else if("GET_ISSUE_BY_USER_ID".equalsIgnoreCase(queryId)){
            ...
            Expression e1 = new  Expression(IssueUserMap.FIELD_USER_ID, REL_OP.EQ, user_id);
            BaseResource[] issueUser = IssueUserMapHelper.getInstance().getByExpression(e1);

            ArrayList<BaseResource> userList = new ArrayList<>();
            for (BaseResource iu : issueUser) {
                IssueUserMap issueusermap = (IssueUserMap) iu;
                Expression e2 = new Expression(Issue.FIELD_ID, REL_OP.EQ, issueusermap.getIssue_id());
