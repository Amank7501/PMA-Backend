...
import com.rasp.app.helper.LabelHelper;
...
public class LabelTest {
    ...
    void updateLabel() throws Exception {
        ...
        String project_id=label.getProject_id();
        String id=label.getId();
        String name=label.getName();
        String color=label.getColor();
        String labelRequestBody = String.format(
                "{\"id\":\"%s\", \"name\":\"new name\",\"color\":\"%s\",\"project_id\":\"%s\"}",
                id,color,project_id
        );
        String encodedResource = Base64.getEncoder().encodeToString(labelRequestBody.getBytes());
        mockMvc.perform(post("/api/label?action=MODIFY")
                        .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                        .content("resource=" + URLEncoder.encode(encodedResource, StandardCharsets.UTF_8))
                        .header("Authorization", "Bearer " + accessToken)
                        .cookie(new Cookie("refresh_token", refreshToken))
    ...
    void deleteProject() throws Exception {
        ...
        String encodedResource = Base64.getEncoder().encodeToString(projectRequestBody.getBytes());
        mockMvc.perform(post("/api/label?action=DELETE")
                        .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                        .content("resource=" + URLEncoder.encode(encodedResource, StandardCharsets.UTF_8))
                        .header("Authorization", "Bearer " + accessToken)
                        .cookie(new Cookie("refresh_token", refreshToken))
